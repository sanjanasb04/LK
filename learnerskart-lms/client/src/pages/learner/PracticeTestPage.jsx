...
const PracticeTest = () => {
  ...
  const handleStartClick = (setName) => {
    ...
    const isFreeRegistrationPractice = nameUpper.includes('PRACTICE TEST (180 QUESTIONS)') ||
                                      setName === 'PRACTICE TEST (180 QUESTIONS)';

    const isDemo = nameUpper.includes('DEMO') || nameUpper.includes('60 QUESTIONS');

    const isPaid = !isDemo && !isFreeRegistrationPractice;

    if (isFreeRegistrationPractice) {
      setDetailsTargetTest(setName);
      setShowDetailsModal(true);
    } else if (isPaid) {
      const paid = safeGetItem(`practice_paid_${setName}`) === 'true' || safeGetItem('mock_paid_bulk_pack') === 'true';
      if (paid) {
        startSet(setName);
      } else {
  ...
  const fetchSets = async () => {
    ...
    try {
      ...
      let fetchedSets = [];
      if (res.data && res.data.success && Array.isArray(res.data.sets) && res.data.sets.length > 0) {
        fetchedSets = [...res.data.sets];
      } else {
        fetchedSets = [
          { name: 'PRACTICE TEST (180 QUESTIONS)', count: 180 },
          { name: 'PRACTICE TEST PAPER FROM PMI 01', count: 180 },
          { name: 'PRACTICE TEST PAPER FROM PMI 02', count: 180 },
          { name: 'PRACTICE TEST PAPER FROM PMI 03', count: 180 },
          { name: 'PRACTICE TEST PAPER FROM PMI 04', count: 180 },
          { name: 'PRACTICE TEST PAPER FROM PMI 05', count: 180 }
        ];
      }
      ...
      let deletedTitles = [];
      try {
        deletedTitles = JSON.parse(localStorage.getItem('deleted_quiz_titles') || '[]').map(t => t.toUpperCase());
      } catch (e) {}

      const finalSets = fetchedSets.filter(s => {
        const nameUpper = (s.name || s.title || '').trim().toUpperCase();
        return !deletedTitles.includes(nameUpper);
      });

      setSets(finalSets);
    } catch (err) {
      console.warn('API fetch warning, using default practice sets:', err.message);
      let deletedTitles = [];
      try {
        deletedTitles = JSON.parse(localStorage.getItem('deleted_quiz_titles') || '[]').map(t => t.toUpperCase());
      } catch (e) {}
      ...
      ].filter(s => !deletedTitles.includes(s.name.toUpperCase()));
  ...
  useEffect(() => {
    ...
    window.addEventListener('quiz_deleted', handleQuizDeleted);
    window.addEventListener('storage', handleQuizDeleted);
    return () => {
      window.removeEventListener('quiz_deleted', handleQuizDeleted);
      window.removeEventListener('storage', handleQuizDeleted);
    };
  }, [testState]);
  ...
  const startSet = async (setName) => {
    ...
    try {
      ...
      console.warn('API fetch warning, using generated practice questions for:', setName);
      const count = 180;
      const fallbackQuestions = Array.from({ length: count }, (_, i) => ({
        _id: `q_prac_${i + 1}`,
        question: `[${setName} - Question ${i + 1}] A project team member discovers a non-conformance issue during quality testing. What is the FIRST document the project manager should consult?`,
        options: {
        ...
        explanation: 'The Quality Management Plan outlines the quality standards, metrics, and non-conformance control procedures for the project.'
      }));
